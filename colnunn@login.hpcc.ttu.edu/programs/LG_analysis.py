...
THRESHOLD = 225  # Minimum ADC value for LOW GAIN event selection
...
def main():
    ...
    for file_path, run_number in run_info:
        ...
        print(f"[INFO] Starting low gain analysis for run {run_number} with threshold {THRESHOLD}, events = {n_entries} remap={True}.")
        if os.path.isfile(HITS_CSV):
            check_run()
        else:
            analyze_run(events=n_entries, max_hits=3, remap=True, board1=BOARD1, board2=BOARD2, board3=BOARD3, threshold=THRESHOLD, veto_threshold=13.6)
